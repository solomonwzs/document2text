...
#include "msoffice/ms_xls/crypt/XORCrypt.h"

typedef unsigned char SVBT16[2];
inline void ShortToSVBT16(unsigned short n, SVBT16 p) {
  p[0] = (unsigned char)n;
...
inline void lclRotateLeft(Type& rnValue, int nBits) {
  ...
  rnValue = static_cast<Type>((rnValue << nBits) |
                              (rnValue >> (sizeof(Type) * 8 - nBits)));
}

template <typename Type>
inline void lclRotateLeft(Type& rnValue, unsigned char nBits,
...
XORCrypt::XORCrypt(int type, unsigned short key, unsigned short hash,
                   std::wstring password)
    : m_nOffset(0), m_nKey(0), m_nHash(0) {
  if (type == 1)
    m_nRotateDistance = 7;  // doc
  else if (type == 2)
    m_nRotateDistance = 2;  // xls
  ...
  static const unsigned char spnFillChars[] = {0xBB, 0xFF, 0xFF, 0xBA, 0xFF,
                                               0xFF, 0xB9, 0x80, 0x00, 0xBE,
                                               0x0F, 0x00, 0xBF, 0x0F, 0x00};
...
void XORCrypt::Decrypt(char* data, const size_t size,
  ...
  if (m_nRotateDistance == 7) {
    for (const unsigned char* pnDataEnd = pnData + size; pnData < pnDataEnd;
         ++pnData) {
      const unsigned char cChar = *pnData ^ *pnCurrKey;
      if (*pnData && cChar) *pnData = cChar;
