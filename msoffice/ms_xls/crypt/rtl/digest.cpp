 ...
 *
 */
#include "msoffice/ms_xls/crypt/rtl/digest.h"
...
void DigestMD5::InitContext() {
  ...
  m_context.m_nA = (uint32_t)0x67452301L;
  m_context.m_nB = (uint32_t)0xefcdab89L;
  m_context.m_nC = (uint32_t)0x98badcfeL;
  m_context.m_nD = (uint32_t)0x10325476L;
}
...
void DigestMD5::EndContext() {
  ...
  switch (m_context.m_nDatLen & 0x03) {
    case 0:
      X[i] = ((unsigned int)(*(p++))) << 0L;
    case 1:
      X[i] |= ((unsigned int)(*(p++))) << 8L;
    case 2:
      X[i] |= ((unsigned int)(*(p++))) << 16L;
    case 3:
      X[i] |= ((unsigned int)(*(p++))) << 24L;
  }

  i += 1;

  if (i >= (DIGEST_LBLOCK_MD5 - 2)) {
    for (; i < DIGEST_LBLOCK_MD5; i++) X[i] = 0;
    UpdateContext();
    i = 0;
  }

  for (; i < (DIGEST_LBLOCK_MD5 - 2); i++) X[i] = 0;
...
DigestMD5::rtlDigestError DigestMD5::Update(const void *pData,
                                            const unsigned int nDatLen) {
  if (nDatLen == 0) return rtl_Digest_E_None;
  ...
  if (m_context.m_nDatLen) {
    unsigned char *p =
        (unsigned char *)(m_context.m_pData) + m_context.m_nDatLen;
...
DigestMD5::rtlDigestError DigestMD5::Get(unsigned char *pBuffer,
                                         const unsigned int nBufLen) {
  unsigned char *p = pBuffer;

  if ((pBuffer == NULL)) return rtl_Digest_E_Argument;

  if (!(RTL_DIGEST_LENGTH_MD5 <= nBufLen)) return rtl_Digest_E_BufferSize;
...
DigestMD5::rtlDigestError DigestMD5::Raw(unsigned char *pBuffer,
                                         const unsigned int nBufLen) {
  unsigned char *p = pBuffer;
