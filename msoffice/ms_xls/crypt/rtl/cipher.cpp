...
#include "msoffice/ms_xls/crypt/rtl/cipher.h"

#include <memory>

CipherARCFOUR::CipherARCFOUR() : m_direction(rtl_Cipher_DirectionInvalid) {}

CipherARCFOUR::rtlCipherError CipherARCFOUR::Init(
    CipherARCFOUR::rtlCipherDirection Direction, const unsigned char *pKeyData,
    const size_t nKeyLen, const unsigned char *pArgData, const size_t nArgLen) {
  if (pKeyData == NULL) {
  ...
  for (x = 0; x < CIPHER_CBLOCK_ARCFOUR; x++) S[x] = x;
...
CipherARCFOUR::rtlCipherError CipherARCFOUR::Encode(const void *pvData,
                                                    const size_t nDatLen,
                                                    unsigned char *pBuffer,
                                                    const size_t nBufLen) {
...
CipherARCFOUR::rtlCipherError CipherARCFOUR::Decode(const void *pvData,
                                                    const size_t nDatLen,
                                                    unsigned char *pBuffer,
                                                    const size_t nBufLen) {
  if (m_direction == rtl_Cipher_DirectionInvalid) {
...
CipherARCFOUR::rtlCipherError CipherARCFOUR::Update(const unsigned char *pData,
                                                    const size_t nDatLen,
                                                    unsigned char *pBuffer,
                                                    const size_t nBufLen) {
  uint32_t *S;
  ...
  /* Check arguments. */
  if ((pData == NULL) || (pBuffer == NULL)) return rtl_Cipher_E_Argument;

  if (!((0 < nDatLen) && (nDatLen <= nBufLen))) return rtl_Cipher_E_BufferSize;
