 ...
 *
 */
#include "msoffice/ms_xls/crypt/BiffDecoder_RCF.h"
...
void BiffDecoderBase::decode(unsigned char* pnDestData,
                             const unsigned char* pnSrcData,
                             const unsigned short nBytes,
                             const long block_index) {
...
BiffDecoder_RCF::BiffDecoder_RCF(unsigned char pnSalt[16],
...
void BiffDecoder_RCF::implDecode(unsigned char* pnDestData,
                                 const unsigned char* pnSrcData,
                                 const unsigned short nBytes,
                                 const long block_index) {
...
void BiffDecoder_RCF::implDecode(unsigned char* pnDestData,
  ...
  while (nBytesLeft > 0) {
    ...
    // decode the block
    unsigned short nBlockLeft =
        static_cast<unsigned short>(block_size - offset);
    unsigned short nDecBytes =
        nBytesLeft < nBlockLeft ? nBytesLeft : nBlockLeft;
