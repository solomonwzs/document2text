...
#include "msoffice/ms_xls/crypt/BinaryCodec_RCF.h"
...
void BinaryCodec_RCF::initKey(const unsigned short pnPassData[16],
                              const unsigned char pnSalt[16]) {
  ...
  size_t nPassSize = 0;
  for (; (pnCurrPass < pnPassEnd) && (*pnCurrPass != 0);
       ++pnCurrPass, ++nPassSize) {
...
bool BinaryCodec_RCF::verifyKey(const unsigned char pnVerifier[16],
                                const unsigned char pnVerifierHash[16]) {
...
bool BinaryCodec_RCF::startBlock(size_t nCounter) {
  ...
  CipherARCFOUR::rtlCipherError eResult =
      mhCipher.Init(CipherARCFOUR::rtl_Cipher_DirectionDecode, pnKeyData,
                    DigestMD5::RTL_DIGEST_LENGTH_MD5, 0, 0);
...
bool BinaryCodec_RCF::decode(unsigned char* pnDestData,
                             const unsigned char* pnSrcData, size_t nBytes) {
  return CipherARCFOUR::rtl_Cipher_E_None ==
         mhCipher.Decode(pnSrcData, static_cast<size_t>(nBytes), pnDestData,
...
bool BinaryCodec_RCF::skip(size_t nBytes) {
  ...
  while (bResult && (nBytesLeft > 0)) {
    size_t nBlockLen =
        nBytesLeft < sizeof(pnDummy) ? nBytesLeft : sizeof(pnDummy);
