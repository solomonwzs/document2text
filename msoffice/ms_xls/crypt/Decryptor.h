...
typedef std::shared_ptr<Decryptor> DecryptorPtr;

class XORDecryptor : public Decryptor {
 public:
  XORDecryptor(int type, unsigned short key, unsigned short hash,
               std::wstring password);

  virtual void Init(const unsigned long val);

  virtual void Decrypt(char* data, const size_t size,
                       const unsigned long stream_pos, const size_t block_size);
  virtual void Decrypt(char* data, const size_t size,
...
typedef std::shared_ptr<XORDecryptor> XORDecryptorPtr;
...
typedef std::shared_ptr<RC4Decryptor> RC4DecryptorPtr;
